...
test domain</code></pre>`,
      ...
      },
      {
        ...
        duration: "30 min",
        summary:
          "Melakukan review performa dasar setelah LMS live: status PM2, response domain, ukuran asset, compression, cache header, memory, dan checklist optimasi ringan.",      
        order: 5,
        materials: [
          {
            id: "l2l-37-html",
            ...
            type: "html",
            description:
              "Checklist review performa LMS setelah deploy, tanpa mengulang detail Gzip, caching, PM2, dan Nginx yang sudah dibahas sebelumnya.",  
            htmlContent:
              `<h2>Performance Review</h2>
<p>Pada lesson ini, kita akan melakukan review performa dasar setelah LMS berhasil live.</p>
<p>Materi PM2, Nginx, HTTPS, Gzip, caching, security headers, dan environment production sudah dibahas sebelumnya. Di sini kita hanya melakukan checklist akhir agar LMS lebih siap dipakai.</p>

<h3>Tujuan performance review</h3>
<ul>
  <li>Memastikan aplikasi LMS berjalan stabil.</li>
  <li>Memastikan domain dan HTTPS merespons dengan benar.</li>
  <li>Memastikan PM2 tidak menunjukkan error berulang.</li>
  <li>Memastikan asset tidak terlalu berat.</li>
  <li>Memastikan Gzip dan cache header aktif jika sudah dikonfigurasi.</li>
  <li>Mengetahui area yang perlu dioptimasi berikutnya.</li>
</ul>

<h3>Folder kerja command</h3>
<p>Sebagian command review performa bisa dijalankan dari folder mana saja.</p>
<p>Namun untuk project LMS, kita mulai dari folder frontend:</p>
...
<h3>Langkah 1 — Cek status PM2</h3>
<p>Command ini dijalankan di VPS:</p>

<pre><code>pm2 status</code></pre>

<p>Pastikan process <code>lms</code> berstatus <code>online</code>.</p>
<p>Jika PM2 belum tersedia:</p>
...
<h3>Langkah 2 — Cek log error singkat</h3>
<p>Kita tidak perlu membaca semua log. Cukup cek bagian terbaru:</p>

<pre><code>pm2 logs lms --lines 80</code></pre>

<p>Untuk keluar dari log:</p>

<pre><code>CTRL + C</code></pre>

<p>Jika tidak ada error berulang, berarti aplikasi relatif stabil.</p>

<h3>Langkah 3 — Cek resource realtime</h3>
<p>Gunakan PM2 monit untuk melihat CPU dan memory:</p>
...
<p>Jika memory terus naik tanpa turun, itu bisa menjadi tanda perlu investigasi lebih lanjut.</p>

<h3>Langkah 4 — Cek response aplikasi internal</h3>
<p>Command ini dijalankan di VPS:</p>

<pre><code>curl -I http://localhost:3000</code></pre>
...
<p>Jika response muncul, berarti aplikasi internal masih merespons.</p>

<h3>Langkah 5 — Cek response domain production</h3>
<p>Cek domain HTTPS:</p>

<pre><code>curl -I https://domainkita.com</code></pre>

<p>Response yang umum dan sehat bisa berupa <code>200 OK</code>, <code>301</code>, atau <code>308</code>, tergantung konfigurasi redirect.</p>

<h3>Langkah 6 — Cek waktu response sederhana</h3>
<p>Untuk melihat waktu response dasar dari domain:</p>

<pre><code>curl -o /dev/null -s -w "time_total: %{time_total}\\n" https://domainkita.com</code></pre>

<p>Command ini bukan benchmark penuh, tetapi cukup membantu untuk cek cepat apakah response terasa sangat lambat.</p>

<h3>Langkah 7 — Cek ukuran folder build Next.js</h3>
<p>Command ini dijalankan di folder frontend:</p>

<pre><code>cd /var/www/lms/frontend
du -sh .next</code></pre>

<p>Jika folder <code>.next</code> belum ada, berarti aplikasi belum pernah dibuild:</p>

<pre><code>npm run build</code></pre>

<h3>Langkah 8 — Cek ukuran asset public</h3>
<p>Cek ukuran folder public:</p>

<pre><code>cd /var/www/lms/frontend
du -sh public</code></pre>

<p>Cek file besar di public:</p>

<pre><code>find public -type f -printf "%s %p\\n" | sort -nr | head -20</code></pre>

<p>Jika command <code>find</code> tidak tersedia, biasanya sudah termasuk di Ubuntu. Jika ada kendala, gunakan:</p>

<pre><code>ls -lh public</code></pre>

<h3>Langkah 9 — Review gambar besar</h3>
<p>Untuk website LMS, gambar hero dan course card sering menjadi penyebab halaman berat.</p>
<p>Target praktis:</p>
<ul>
  <li>Hero image: usahakan tidak terlalu besar.</li>
  <li>Card image: gunakan ukuran sesuai kebutuhan tampilan.</li>
  <li>Gunakan format modern seperti <code>webp</code> jika memungkinkan.</li>
  <li>Hindari upload gambar mentah ukuran kamera langsung ke production.</li>
</ul>

<h3>Langkah 10 — Cek Gzip secara singkat</h3>
<p>Detail Gzip sudah dibahas di lesson sebelumnya. Di sini cukup validasi header:</p>

<pre><code>curl -I -H "Accept-Encoding: gzip" https://domainkita.com</code></pre>

<p>Jika aktif, cari header:</p>

<pre><code>Content-Encoding: gzip</code></pre>

<h3>Langkah 11 — Cek cache header secara singkat</h3>
<p>Detail caching juga sudah dibahas sebelumnya. Di sini cukup cek asset statis:</p>

<pre><code>curl -I https://domainkita.com/favicon.ico</code></pre>

<p>Jika cache aktif, biasanya ada header:</p>

<pre><code>Cache-Control: public</code></pre>

<h3>Langkah 12 — Cek security headers secara singkat</h3>
<p>Security headers sudah dibahas sebelumnya. Di sini cukup validasi:</p>

<pre><code>curl -I https://domainkita.com</code></pre>

<p>Cari header seperti:</p>
...
<h3>Langkah 13 — Cek kapasitas disk VPS</h3>
<p>Command ini dijalankan di VPS:</p>

<pre><code>df -h</code></pre>

<p>Jika disk hampir penuh, aplikasi bisa bermasalah saat build, install dependency, atau menulis log.</p>

<h3>Langkah 14 — Cek memory VPS</h3>
<p>Command ini dijalankan di VPS:</p>

<pre><code>free -h</code></pre>

<p>Jika memory sangat kecil dan sering penuh, pertimbangkan optimasi aplikasi, mengurangi process, atau upgrade VPS.</p>

<h3>Langkah 15 — Checklist final</h3>
<table>
  <thead>
    <tr>
      <th>Area</th>
  ...
  <tbody>
    ...
    <tr>
      <td>Domain</td>
      <td><code>curl -I https://domainkita.com</code></td>
      <td>Ada response HTTPS</td>
    </tr>
    <tr>
      <td>Gzip</td>
      <td><code>curl -I -H "Accept-Encoding: gzip" ...</code></td>
      <td>Ada header gzip jika aktif</td>
    </tr>
    <tr>
      <td>Cache</td>
      <td><code>curl -I /favicon.ico</code></td>
      <td>Ada Cache-Control jika aktif</td>
    </tr>
    <tr>
      <td>Disk</td>
      <td><code>df -h</code></td>
      <td>Masih ada ruang cukup</td>
    </tr>
    <tr>
      <td>Memory</td>
      <td><code>free -h</code></td>
      <td>Tidak penuh terus-menerus</td>
    </tr>
  </tbody>
</table>
...
<h4>1. Website terasa lambat</h4>
<p>Cek waktu response:</p>

<pre><code>curl -o /dev/null -s -w "time_total: %{time_total}\\n" https://domainkita.com</code></pre>

<p>Lalu cek PM2 dan resource:</p>
...
<p>Biasanya aplikasi internal mati atau port 3000 tidak merespons:</p>

<pre><code>pm2 status
curl -I http://localhost:3000
pm2 logs lms --lines 100</code></pre>

<h4>3. Gambar terlalu besar</h4>
<p>Cari file besar:</p>

<pre><code>cd /var/www/lms/frontend
find public -type f -printf "%s %p\\n" | sort -nr | head -20</code></pre>

<p>Solusi umum: resize gambar, gunakan WebP, dan pakai ukuran gambar sesuai kebutuhan tampilan.</p>

<h4>4. Build terlalu berat atau lambat</h4>
<p>Cek disk dan memory:</p>

<pre><code>df -h
free -h</code></pre>

<p>Jika resource terlalu kecil, build bisa gagal atau sangat lambat.</p>

<h4>5. Header Gzip atau Cache tidak muncul</h4>
<p>Jangan ulang konfigurasi dari awal. Cukup cek ulang lesson Gzip dan Caching, lalu validasi Nginx:</p>

<pre><code>sudo nginx -t
sudo systemctl reload nginx</code></pre>

<h4>6. PM2 restart count terus naik</h4>
<p>Ini tanda aplikasi crash berulang.</p>

<pre><code>pm2 show lms
pm2 logs lms --lines 150</code></pre>

<p>Fokus ke error pertama sebelum aplikasi restart.</p>

<h3>Ringkasan command</h3>

<pre><code>cd /var/www/lms/frontend
pm2 status
pm2 logs lms --lines 80
pm2 monit
curl -I http://localhost:3000
curl -I https://domainkita.com
...
du -sh public
find public -type f -printf "%s %p\\n" | sort -nr | head -20
curl -I -H "Accept-Encoding: gzip" https://domainkita.com
curl -I https://domainkita.com/favicon.ico
df -h
free -h</code></pre>

<h3>Kesimpulan</h3>
<p>Pada lesson ini, kita melakukan review performa dasar setelah LMS live.</p>
<p>Kita tidak mengulang konfigurasi yang sudah dibahas, tetapi melakukan validasi akhir: PM2 online, domain merespons, HTTPS aktif, asset tidak terlalu berat, Gzip/cache/header berjalan, serta resource VPS masih aman.</p>
<p>Performance review sebaiknya dilakukan berkala, terutama setelah update code, upload gambar baru, atau perubahan konfigurasi server.</p>`,
          },
          {
            id: "l2l-37-video",
            ...
            type: "video",
            description:
              "Video pendamping untuk memahami checklist review performa LMS setelah live di VPS.",
            url: "https://youtu.be/uOeAt_woF_c?si=v5zNPGajvaOKYyrJ",
            duration: "30 min", 
          },
        ],
    ...
    lessons: [
      {
        ...
        duration: "32 min",
        summary:
          "Membuat backup source code LMS secara manual dan dengan Git, serta memahami strategi backup dasar agar aman saat terjadi error atau update gagal.",  
        order: 1,
        materials: [
          {
            id: "l2l-38-html",
            ...
            type: "html",
            description:
              "Melakukan backup source code dengan copy manual dan Git, cek hasil backup, serta troubleshooting jika backup gagal.",   
            htmlContent:
              `<h2>Backup Source Code</h2>
<p>Pada lesson ini, kita akan membahas cara melakukan backup source code LMS.</p>
<p>Materi Git dan deployment sudah dibahas sebelumnya, jadi di sini kita fokus pada strategi backup sederhana agar project tetap aman.</p>

<h3>Kenapa backup itu penting?</h3>
<ul>
  <li>Mencegah kehilangan data saat server error.</li>
  <li>Memudahkan rollback jika update gagal.</li>
  <li>Menjaga versi stable tetap aman.</li>
  <li>Melindungi dari human error (hapus file, overwrite, dll).</li>
</ul>

<h3>Jenis backup yang akan kita bahas</h3>
<ul>
  <li>Backup manual (copy folder)</li>
  <li>Backup dengan Git (best practice)</li>
</ul>

<h3>Folder kerja command</h3>
<p>Untuk backup source code LMS, kita bekerja di folder project:</p>

<pre><code>cd /var/www/lms
pwd</code></pre>

<h3>Langkah 1 — Backup manual (copy folder)</h3>
<p>Command ini dijalankan di VPS:</p>

<pre><code>cd /var/www
cp -r lms lms-backup-$(date +%Y%m%d-%H%M)</code></pre>

<p>Command ini akan membuat folder backup dengan timestamp, misalnya:</p>

<pre><code>lms-backup-20260427-1030</code></pre>

<p>Cek hasil backup:</p>

<pre><code>ls -la /var/www</code></pre>

<h3>Langkah 2 — Backup dengan archive (lebih hemat space)</h3>
<p>Alternatif backup menggunakan file archive:</p>

<pre><code>cd /var/www
tar -czf lms-backup-$(date +%Y%m%d-%H%M).tar.gz lms</code></pre>

<p>Cek hasilnya:</p>

<pre><code>ls -lh /var/www</code></pre>

<p>File <code>.tar.gz</code> biasanya lebih kecil dibanding copy folder langsung.</p>

<h3>Langkah 3 — Restore dari backup</h3>
<p>Jika terjadi error, kita bisa restore:</p>

<pre><code>cd /var/www
rm -rf lms
cp -r lms-backup-20260427-1030 lms</code></pre>

<p>Atau jika memakai archive:</p>

<pre><code>tar -xzf lms-backup-20260427-1030.tar.gz</code></pre>

<h3>Langkah 4 — Backup dengan Git (recommended)</h3>
<p>Best practice adalah menyimpan source code di Git repository.</p>
...
<p>Jika Git belum tersedia:</p>

<pre><code>sudo apt update
sudo apt install git -y</code></pre>

<h3>Langkah 5 — Commit perubahan</h3>

<pre><code>cd /var/www/lms
git add .
git commit -m "backup before update"</code></pre>

<h3>Langkah 6 — Push ke remote repository</h3>

<pre><code>git push origin main</code></pre>

<p>Dengan Git, backup kita tidak hanya di VPS, tetapi juga tersimpan di GitHub/GitLab.</p>

<h3>Langkah 7 — Workflow aman sebelum update</h3>

<pre><code># sebelum update
git add .
git commit -m "before update"
git push

# optional backup manual
cp -r /var/www/lms /var/www/lms-backup-safe</code></pre>

<h3>Langkah 8 — Checklist backup aman</h3>
<ul>
  <li>Source code sudah di Git (recommended).</li>
  <li>Ada backup manual sebelum perubahan besar.</li>
  <li>Backup memiliki timestamp.</li>
  <li>Backup bisa direstore dengan cepat.</li>
</ul>

<h3>Catatan penting</h3>
<ul>
  <li>Backup source code ≠ backup database (jika ada backend DB).</li>
  <li>Backup manual cocok untuk quick safety.</li>
  <li>Git adalah backup terbaik untuk code versioning.</li>
  <li>Jangan hanya mengandalkan satu jenis backup.</li>
</ul>

<h3>Troubleshooting</h3>

<h4>1. cp: command not found</h4>
<p>Biasanya sudah tersedia di Linux. Jika tidak:</p>

<pre><code>which cp</code></pre>

<h4>2. Permission denied saat copy</h4>

<pre><code>sudo cp -r lms lms-backup</code></pre>

<h4>3. Folder backup tidak muncul</h4>

<pre><code>ls -la /var/www</code></pre>

<p>Pastikan command dijalankan di folder yang benar.</p>

<h4>4. Git push gagal</h4>
<p>Masalah umum:</p>
<ul>
  <li>SSH key belum setup.</li>
  <li>Repository tidak ada.</li>
  <li>Permission GitHub/GitLab.</li>
</ul>
...
<h4>5. Backup terlalu besar</h4>
<p>Gunakan archive:</p>

<pre><code>tar -czf backup.tar.gz lms</code></pre>

<h4>6. Salah restore backup</h4>
<p>Pastikan folder target benar sebelum overwrite:</p>
...
<h3>Kesimpulan</h3>
<p>Pada lesson ini, kita menambahkan layer keamanan tambahan berupa backup source code.</p>
<p>Minimal gunakan Git sebagai backup utama, dan backup manual sebelum perubahan besar.</p>
<p>Rule sederhana:</p>

<pre><code>before change → backup
after change → commit</code></pre>`,
          },
          {
            id: "l2l-38-video",
            title: "Video Backup Source Code",
            type: "video",
            description:
              "Video pendamping untuk memahami strategi backup source code LMS di VPS.",
            url: "https://youtu.be/uOeAt_woF_c?si=v5zNPGajvaOKYyrJ",
            duration: "32 min",    
          },
        ],
      },
      {
        ...
        duration: "30 min",
        summary:
          "Mempersiapkan strategi backup database sejak awal agar LMS siap berkembang dari frontend live menjadi aplikasi fullstack yang menyimpan data penting.",   
        order: 2,
        materials: [
          {
            id: "l2l-39-html",
            ...
            type: "html",
            description:
              "Memahami rencana backup database, command dasar PostgreSQL/MySQL, lokasi penyimpanan backup, validasi hasil backup, dan troubleshooting umum.",  
            htmlContent:
              `<h2>Database Backup Planning</h2>
<p>Pada lesson ini, kita akan membahas perencanaan backup database untuk LMS.</p>
<p>Jika LMS kita saat ini masih fokus pada frontend atau SSR tanpa database sendiri, maka backup utama masih source code dan file environment. Tetapi begitu LMS mulai menyimpan user, transaksi, progress belajar, order, payment, quiz, atau data member, backup database menjadi wajib.</p>

<h3>Kenapa database backup penting?</h3>
<ul>
  <li>Database menyimpan data yang tidak bisa diganti hanya dengan Git.</li>
  <li>Source code bisa diambil ulang dari repository, tetapi data user tidak.</li>
  <li>Backup membantu recovery saat salah update, server rusak, database corrupt, atau human error.</li>
  <li>Backup memberi rasa aman sebelum melakukan perubahan besar.</li>
</ul>

<h3>Apa bedanya backup source code dan database?</h3>
<table>
  <thead>
    <tr>
      <th>Jenis backup</th>
  ...
  <tbody>
    <tr>
      <td>Source code</td>
      <td>File aplikasi</td>
      <td>React, Next.js, TypeScript, CSS, config project</td>
    </tr>
    <tr>
      <td>Environment</td>
      <td>Konfigurasi rahasia/server</td>
      <td>.env.production, API key, database URL</td>
    </tr>
    <tr>
      <td>Database</td>
      <td>Data aplikasi</td>
      <td>User, course progress, payment, order, enrollment</td>
    </tr>
  </tbody>
</table>

<h3>Folder kerja command</h3>
<p>Untuk backup database, command biasanya boleh dijalankan dari folder mana saja.</p>
<p>Namun agar rapi, kita buat folder khusus backup:</p>

<pre><code>sudo mkdir -p /var/backups/lms
sudo chown -R $USER:$USER /var/backups/lms
cd /var/backups/lms
pwd</code></pre>

<p>Jika hanya ingin cek project LMS, gunakan:</p>

<pre><code>cd /var/www/lms
pwd</code></pre>

<h3>Langkah 1 — Identifikasi database yang digunakan</h3>
<p>Sebelum membuat backup, kita perlu tahu LMS memakai database apa.</p>
<p>Cek file environment production:</p>

<pre><code>cd /var/www/lms/frontend
ls -la | grep env
cat .env.production</code></pre>

<p>Jika ada variable seperti <code>DATABASE_URL</code>, lihat jenis databasenya:</p>

<pre><code>postgresql://...
mysql://...</code></pre>

<p><strong>Catatan:</strong> hati-hati saat menampilkan file env karena bisa berisi secret. Jangan screenshot atau share ke publik.</p>

<h3>Langkah 2 — Cek apakah PostgreSQL terinstall</h3>
<p>Jika nanti LMS memakai PostgreSQL, cek client tool:</p>

<pre><code>psql --version
pg_dump --version</code></pre>

<p>Jika belum tersedia:</p>
...
<h3>Langkah 3 — Contoh backup PostgreSQL</h3>
<p>Command ini dijalankan dari folder backup:</p>

<pre><code>cd /var/backups/lms
pg_dump -h localhost -U nama_user_db -d nama_database &gt; lms-db-$(date +%Y%m%d-%H%M).sql</code></pre>

<p>Jika ingin hasil backup langsung compressed:</p>

<pre><code>cd /var/backups/lms
pg_dump -h localhost -U nama_user_db -d nama_database | gzip &gt; lms-db-$(date +%Y%m%d-%H%M).sql.gz</code></pre>

<p>Cek hasil backup:</p>

<pre><code>ls -lh /var/backups/lms</code></pre>

<h3>Langkah 4 — Cek apakah MySQL/MariaDB client tersedia</h3>
<p>Jika nanti LMS memakai MySQL atau MariaDB:</p>

<pre><code>mysql --version
mysqldump --version</code></pre>

<p>Jika belum tersedia:</p>

<pre><code>sudo apt update
sudo apt install mysql-client -y</code></pre>

<h3>Langkah 5 — Contoh backup MySQL/MariaDB</h3>
<p>Command ini dijalankan dari folder backup:</p>

<pre><code>cd /var/backups/lms
mysqldump -u nama_user_db -p nama_database &gt; lms-db-$(date +%Y%m%d-%H%M).sql</code></pre>

<p>Versi compressed:</p>

<pre><code>cd /var/backups/lms
mysqldump -u nama_user_db -p nama_database | gzip &gt; lms-db-$(date +%Y%m%d-%H%M).sql.gz</code></pre>

<p>Cek hasil backup:</p>

<pre><code>ls -lh /var/backups/lms</code></pre>

<h3>Langkah 6 — Validasi file backup</h3>
<p>Backup tidak cukup dibuat. Kita juga perlu memastikan file-nya benar-benar ada dan tidak kosong.</p>

<pre><code>cd /var/backups/lms
ls -lh
du -sh *</code></pre>

<p>Jika file backup ukurannya <code>0</code> byte, berarti backup gagal atau database kosong.</p>

<h3>Langkah 7 — Simpan backup di lokasi berbeda</h3>
<p>Backup yang hanya ada di VPS yang sama masih berisiko. Jika VPS rusak, backup ikut hilang.</p>
<p>Minimal, backup penting sebaiknya disalin ke:</p>
<ul>
  <li>PC lokal</li>
  <li>Storage lain</li>
  <li>Object storage</li>
  <li>Server backup khusus</li>
</ul>

<p>Contoh download backup dari PC lokal menggunakan <code>scp</code>:</p>

<pre><code>scp deploy@IP_VPS:/var/backups/lms/nama-file-backup.sql.gz .</code></pre>

<p>Command di atas dijalankan dari PC lokal, bukan dari VPS.</p>

<h3>Langkah 8 — Rencana jadwal backup</h3>
<p>Untuk tahap awal, gunakan pola sederhana:</p>
<ul>
  <li>Sebelum update besar: backup manual.</li>
  <li>Setelah data user aktif: backup harian.</li>
  <li>Jika transaksi/payment aktif: pertimbangkan backup lebih sering.</li>
  <li>Simpan beberapa versi backup, bukan hanya satu file terakhir.</li>
</ul>

<h3>Langkah 9 — Jangan backup secret sembarangan</h3>
<p>File database backup bisa berisi data sensitif seperti email user, transaksi, progress belajar, atau token tertentu.</p>
<p>Karena itu:</p>
<ul>
  <li>Jangan upload backup database ke repository Git.</li>
  <li>Jangan share backup database di chat publik.</li>
  <li>Batasi permission folder backup.</li>
  <li>Gunakan storage yang aman.</li>
</ul>

<h3>Langkah 10 — Permission dasar folder backup</h3>
<p>Batasi akses folder backup:</p>

<pre><code>sudo chmod 700 /var/backups/lms
ls -ld /var/backups/lms</code></pre>

<p>Dengan permission <code>700</code>, hanya owner folder yang bisa membaca, menulis, dan masuk ke folder tersebut.</p>

<h3>Catatan penting untuk fase LMS saat ini</h3>
<p>Jika LMS kita belum memakai database sendiri, lesson ini cukup menjadi planning dan checklist masa depan.</p>
<p>Jangan memaksa install database hanya untuk mengikuti lesson ini. Install database dilakukan saat arsitektur backend memang sudah membutuhkan database.</p>

<h3>Troubleshooting</h3>

<h4>1. pg_dump: command not found</h4>
...
<h4>2. mysqldump: command not found</h4>
...
<h4>3. Permission denied saat menulis backup</h4>
<p>Pastikan folder backup bisa ditulis oleh user aktif:</p>
...
<h4>4. Authentication failed ke database</h4>
<p>Cek username, password, host, port, dan nama database.</p>
<p>Untuk PostgreSQL, test koneksi:</p>

<pre><code>psql -h localhost -U nama_user_db -d nama_database</code></pre>

<p>Untuk MySQL:</p>

<pre><code>mysql -u nama_user_db -p nama_database</code></pre>

<h4>5. Backup berhasil tetapi file terlalu besar</h4>
<p>Gunakan gzip:</p>

<pre><code>gzip nama-file-backup.sql</code></pre>

<p>Atau backup langsung ke compressed file seperti contoh sebelumnya.</p>

<h4>6. Backup ada, tetapi belum pernah diuji restore</h4>
<p>Backup yang belum pernah diuji restore belum bisa dianggap aman.</p>
<p>Minimal, buat checklist restore untuk lesson berikutnya dan test di environment non-production.</p>
...
pg_dump --version
sudo apt install postgresql-client -y
pg_dump -h localhost -U nama_user_db -d nama_database | gzip &gt; lms-db-$(date +%Y%m%d-%H%M).sql.gz
mysql --version
mysqldump --version
sudo apt install mysql-client -y
mysqldump -u nama_user_db -p nama_database | gzip &gt; lms-db-$(date +%Y%m%d-%H%M).sql.gz
ls -lh /var/backups/lms
du -sh /var/backups/lms/*
sudo chmod 700 /var/backups/lms</code></pre>

<h3>Kesimpulan</h3>
<p>Pada lesson ini, kita belum memaksa LMS memakai database. Kita menyiapkan mindset dan strategi backup database agar saat LMS berkembang menjadi fullstack, data penting sudah punya rencana perlindungan.</p>
<p>Rule pentingnya sederhana:</p>

<pre><code>source code → backup dengan Git
environment → backup aman dan private
database → backup rutin, disimpan aman, dan wajib pernah diuji restore</code></pre>`,
          },
          {
            id: "l2l-39-video",
            title: "Video Database Backup Planning",
            type: "video",
            description:
              "Video pendamping untuk memahami rencana backup database saat LMS berkembang menjadi aplikasi fullstack.",
            url: "https://youtu.be/uOeAt_woF_c?si=v5zNPGajvaOKYyrJ",
            duration: "30 min",    
          },
        ],
      },
      {
        ...
        duration: "30 min",
        summary:
          "Memantau apakah LMS tetap online setelah deployment menggunakan pengecekan manual, curl, PM2, Nginx, dan uptime monitoring eksternal.", 
        order: 3,
        materials: [
          {
            id: "l2l-40-html",
            ...
            type: "html",
            description:
              "Memahami uptime monitoring, cek status website, cek response domain, cek service, dan troubleshooting jika LMS down.",  
            htmlContent:
              `<h2>Uptime Monitoring</h2>
<p>Pada lesson ini, kita akan membahas cara memantau apakah LMS tetap online setelah deployment.</p>
<p>Materi PM2 Monitoring sudah membahas status process aplikasi. Di lesson ini, fokusnya adalah memantau apakah website benar-benar bisa diakses dari luar oleh user.</p>

<h3>Apa itu uptime?</h3>
<p><strong>Uptime</strong> adalah kondisi ketika website atau aplikasi sedang online dan bisa diakses oleh user.</p>
<p>Jika LMS tidak bisa diakses, maka itu disebut downtime.</p>

<h3>Kenapa uptime monitoring penting?</h3>
<ul>
  <li>Kita bisa tahu lebih cepat jika website down.</li>
  <li>Kita bisa membedakan apakah masalah ada di domain, Nginx, PM2, aplikasi, atau VPS.</li>
  <li>Website production lebih mudah dirawat.</li>
  <li>User tidak perlu menjadi orang pertama yang melaporkan website error.</li>
</ul>

<h3>Folder kerja command</h3>
<p>Command monitoring uptime bisa dijalankan dari folder mana saja.</p>
<p>Namun untuk konsistensi course ini, kita mulai dari folder frontend LMS:</p>

<pre><code>cd /var/www/lms/frontend
pwd</code></pre>

<p>Jika folder tidak ditemukan, cek struktur project:</p>

<pre><code>ls -la /var/www/lms
ls -la /var/www/lms/frontend</code></pre>

<h3>Langkah 1 — Cek website dari browser</h3>
<p>Cara paling mudah adalah membuka domain dari browser:</p>

<pre><code>https://domainkita.com</code></pre>

<p>Jika halaman LMS tampil, website sedang online dari sisi user.</p>

<h3>Langkah 2 — Cek response domain dari VPS</h3>
<p>Command ini dijalankan di VPS:</p>
...
<p>Response yang umum:</p>
<ul>
  <li><code>200 OK</code> — halaman berhasil diakses.</li>
  <li><code>301</code> atau <code>308</code> — redirect, biasanya dari HTTP ke HTTPS.</li>
  <li><code>502 Bad Gateway</code> — Nginx hidup, tetapi aplikasi di belakangnya bermasalah.</li>
  <li><code>404 Not Found</code> — server merespons, tetapi route/path tidak ditemukan.</li>
  <li><code>500</code> — aplikasi mengalami error internal.</li>
</ul>

<h3>Langkah 3 — Cek response time sederhana</h3>
<p>Untuk melihat waktu response secara sederhana:</p>

<pre><code>curl -o /dev/null -s -w "status: %{http_code}\\ntime_total: %{time_total}\\n" https://domainkita.com</code></pre>

<p>Command ini menampilkan status HTTP dan total waktu request.</p>

<h3>Langkah 4 — Cek dari PC lokal</h3>
<p>Command ini dijalankan dari PC lokal, misalnya PowerShell Windows:</p>

<pre><code>curl -I https://domainkita.com</code></pre>

<p>Jika PowerShell memakai alias curl yang berbeda, gunakan:</p>

<pre><code>curl.exe -I https://domainkita.com</code></pre>

<p>Tujuannya untuk memastikan website bisa diakses dari luar VPS, bukan hanya dari server itu sendiri.</p>

<h3>Langkah 5 — Cek DNS jika domain tidak bisa diakses</h3>
<p>Command ini bisa dijalankan dari PC lokal atau VPS:</p>

<pre><code>nslookup domainkita.com 8.8.8.8</code></pre>

<p>Jika <code>nslookup</code> belum tersedia di VPS:</p>

<pre><code>sudo apt update
sudo apt install dnsutils -y</code></pre>

<p>Hasil yang benar harus mengarah ke IP public VPS.</p>

<h3>Langkah 6 — Cek Nginx</h3>
<p>Jika domain error, cek Nginx:</p>
...
<h3>Langkah 7 — Cek PM2</h3>
<p>Jika Nginx aktif tetapi website menampilkan <code>502 Bad Gateway</code>, cek aplikasi LMS di PM2:</p>
...
<p>Jika process LMS belum berjalan:</p>

<pre><code>cd /var/www/lms/frontend
pm2 start npm --name lms -- start
pm2 save</code></pre>

<h3>Langkah 8 — Cek aplikasi internal</h3>
<p>Command ini dijalankan di VPS:</p>

<pre><code>curl -I http://localhost:3000</code></pre>

<p>Jika tidak ada response, berarti aplikasi internal belum berjalan atau port berbeda dari konfigurasi Nginx.</p>

<h3>Langkah 9 — Cek firewall</h3>
<p>Jika website timeout, cek firewall:</p>
...
<p>Pastikan port web terbuka:</p>

<pre><code>sudo ufw allow 80
sudo ufw allow 443
sudo ufw status</code></pre>

<h3>Langkah 10 — Monitoring eksternal</h3>
<p>Selain cek manual, gunakan layanan uptime monitoring eksternal.</p>
<p>Contoh konsep yang perlu diset:</p>
<ul>
  <li>Monitor type: HTTP/HTTPS</li>
  <li>URL: <code>https://domainkita.com</code></li>
  <li>Interval: 1 menit, 5 menit, atau 10 menit</li>
  <li>Alert: email, Telegram, Discord, Slack, atau webhook</li>
</ul>

<p>Monitoring eksternal penting karena ia mengecek website dari luar server. Jadi jika VPS, DNS, SSL, atau Nginx bermasalah, kita bisa mendapat notifikasi.</p>

<h3>Langkah 11 — Buat checklist saat website down</h3>
<p>Jika LMS tidak bisa diakses, cek urutan berikut:</p>

<pre><code>1. Cek domain dari browser
2. Cek curl -I https://domainkita.com
3. Cek DNS dengan nslookup
4. Cek Nginx
5. Cek PM2
6. Cek localhost:3000
7. Cek firewall
8. Cek log error</code></pre>

<h3>Catatan untuk pemula</h3>
<p>Website bisa terlihat “down” karena banyak sebab. Tidak selalu karena aplikasi rusak.</p>
<p>Bisa saja penyebabnya DNS, SSL, Nginx, firewall, PM2, atau VPS provider.</p>

<h3>Catatan untuk advanced</h3>
<p>Untuk production yang lebih serius, uptime monitoring sebaiknya digabung dengan log monitoring, error tracking, resource monitoring, dan backup alert.</p>
<p>Namun untuk tahap awal course ini, monitoring URL HTTPS sudah cukup sebagai fondasi.</p>

<h3>Troubleshooting</h3>

<h4>1. Website timeout</h4>
<p>Cek DNS dan firewall:</p>

<pre><code>nslookup domainkita.com 8.8.8.8
sudo ufw status
sudo systemctl status nginx</code></pre>

<h4>2. Website menampilkan 502 Bad Gateway</h4>
<p>Biasanya Nginx hidup, tetapi aplikasi LMS di belakangnya mati.</p>
...
<h4>3. HTTPS error</h4>
<p>Cek Certbot dan Nginx:</p>

<pre><code>sudo nginx -t
sudo certbot certificates
sudo systemctl status nginx</code></pre>

<h4>4. Domain mengarah ke IP yang salah</h4>
<p>Cek DNS:</p>

<pre><code>nslookup domainkita.com 8.8.8.8
nslookup www.domainkita.com 8.8.8.8</code></pre>

<p>Perbaiki A Record di DNS panel jika IP belum sesuai VPS.</p>

<h4>5. Uptime monitor eksternal bilang down, tetapi di VPS curl berhasil</h4>
<p>Berarti website mungkin hanya bisa diakses dari dalam VPS, tetapi tidak dari internet.</p>
<p>Cek firewall VPS dan firewall provider:</p>

<pre><code>sudo ufw status</code></pre>

<p>Pastikan port <code>80</code> dan <code>443</code> terbuka di dashboard provider VPS juga.</p>

<h4>6. Uptime monitor terlalu sering memberi false alarm</h4>
<p>Naikkan interval pengecekan atau gunakan beberapa lokasi monitoring jika tersedia.</p>
<p>Untuk tahap awal, interval 5 menit biasanya cukup.</p>
...
curl.exe -I https://domainkita.com
nslookup domainkita.com 8.8.8.8
sudo systemctl status nginx
sudo nginx -t
pm2 status
...
<h3>Kesimpulan</h3>
<p>Pada lesson ini, kita belajar cara memantau uptime LMS setelah live.</p>
<p>PM2 monitoring memberi tahu apakah aplikasi hidup di server, sedangkan uptime monitoring memastikan website bisa diakses oleh user dari luar.</p>
<p>Minimal, lakukan pengecekan manual dengan <code>curl</code> dan gunakan satu uptime monitor eksternal untuk notifikasi jika website down.</p>`,
          },
          {
            id: "l2l-40-video",
            title: "Video Uptime Monitoring",
            type: "video",
            description:
              "Video pendamping untuk memahami uptime monitoring LMS setelah live di VPS.",
            url: "https://youtu.be/uOeAt_woF_c?si=v5zNPGajvaOKYyrJ",
            duration: "30 min",   
          },
        ],
      },
      {
        ...
        duration: "32 min",
        summary:
          "Menyusun langkah restore dasar ketika LMS bermasalah, mulai dari restore source code, environment, dependency, build, PM2, Nginx, hingga validasi domain.",    
        order: 4,
        materials: [
          {
            id: "l2l-41-html",
            ...
            type: "html",
            description:
              "Checklist restore dasar setelah incident, termasuk restore source code, env, build ulang, restart PM2, validasi Nginx, dan troubleshooting recovery.", 
            htmlContent:
              `<h2>Restore Dasar</h2>
<p>Pada lesson ini, kita akan membahas skenario restore dasar ketika LMS bermasalah.</p>
<p>Materi backup source code, database backup planning, PM2, Nginx, HTTPS, dan uptime monitoring sudah dibahas sebelumnya. Di sini kita fokus pada urutan pemulihan agar LMS bisa kembali online.</p>

<h3>Kenapa restore harus dipelajari?</h3>
<p>Backup hanya benar-benar berguna kalau kita tahu cara mengembalikannya.</p>
<p>Restore adalah proses mengembalikan aplikasi ke kondisi yang sehat setelah terjadi masalah seperti update gagal, file terhapus, build error, konfigurasi rusak, atau service mati.</p>

<h3>Target restore dasar</h3>
<ul>
  <li>Mengembalikan source code dari backup atau Git.</li>
  <li>Memastikan file environment tersedia.</li>
  <li>Install dependency ulang jika perlu.</li>
  <li>Build ulang aplikasi.</li>
  <li>Restart PM2.</li>
  <li>Validasi Nginx dan domain.</li>
  <li>Memastikan LMS kembali online.</li>
</ul>

<h3>Folder kerja command</h3>
<p>Untuk restore source code LMS, kita bekerja di folder utama project:</p>

<pre><code>cd /var/www/lms
pwd</code></pre>

<p>Untuk restore aplikasi frontend, kita masuk ke folder frontend:</p>

<pre><code>cd /var/www/lms/frontend
pwd</code></pre>

<p>Jika folder tidak ada, cek:</p>

<pre><code>ls -la /var/www
ls -la /var/www/lms</code></pre>

<h3>Langkah 1 — Jangan panik, cek kondisi awal</h3>
<p>Sebelum restore, cek dulu kondisi server agar kita tahu bagian mana yang rusak.</p>
...
<h3>Langkah 2 — Cek backup yang tersedia</h3>
<p>Jika backup manual dibuat di <code>/var/www</code>, cek:</p>

<pre><code>ls -la /var/www | grep lms-backup</code></pre>

<p>Jika backup archive dibuat, cek:</p>

<pre><code>ls -lh /var/www | grep tar.gz</code></pre>

<p>Jika backup database ada di folder backup:</p>

<pre><code>ls -lh /var/backups/lms</code></pre>

<h3>Langkah 3 — Restore source code dari Git</h3>
<p>Jika source code masih ada dan hanya perlu dikembalikan ke versi remote:</p>

<pre><code>cd /var/www/lms
git status
git pull</code></pre>

<p>Jika folder project rusak berat, buat backup dulu sebelum mengganti:</p>

<pre><code>cd /var/www
mv lms lms-broken-$(date +%Y%m%d-%H%M)</code></pre>

<p>Lalu clone ulang dari repository:</p>

<pre><code>cd /var/www
git clone git@github.com:USERNAME/REPOSITORY.git lms</code></pre>

<p>Sesuaikan URL repository dengan repository LMS kita.</p>

<h3>Langkah 4 — Restore source code dari backup folder</h3>
<p>Jika kita punya backup folder, contoh <code>lms-backup-20260427-1030</code>:</p>

<pre><code>cd /var/www
mv lms lms-broken-$(date +%Y%m%d-%H%M)
cp -r lms-backup-20260427-1030 lms</code></pre>

<p>Cek hasil restore:</p>

<pre><code>ls -la /var/www/lms</code></pre>

<h3>Langkah 5 — Restore dari file archive</h3>
<p>Jika backup berupa <code>.tar.gz</code>:</p>

<pre><code>cd /var/www
mv lms lms-broken-$(date +%Y%m%d-%H%M)
tar -xzf lms-backup-20260427-1030.tar.gz</code></pre>

<p>Jika hasil extract membuat folder dengan nama berbeda, cek:</p>

<pre><code>ls -la /var/www</code></pre>

<h3>Langkah 6 — Pastikan file environment ada</h3>
<p>Setelah source code kembali, cek file environment production.</p>

<pre><code>cd /var/www/lms/frontend
ls -la | grep env</code></pre>

<p>Jika <code>.env.production</code> tidak ada, restore dari backup private atau buat ulang berdasarkan <code>.env.example</code>:</p>

<pre><code>cp .env.example .env.production
nano .env.production</code></pre>

<p><strong>Penting:</strong> isi ulang value production yang benar, seperti domain, API URL, database URL, atau secret jika ada.</p>

<h3>Langkah 7 — Install dependency ulang</h3>
<p>Command ini dijalankan dari folder frontend:</p>

<pre><code>cd /var/www/lms/frontend
npm install</code></pre>

<p>Jika <code>npm</code> belum tersedia:</p>

<pre><code>node -v
npm -v</code></pre>

<p>Jika Node.js/npm belum ada, install Node.js sesuai lesson sebelumnya.</p>

<h3>Langkah 8 — Build ulang aplikasi</h3>
<p>Setelah dependency siap, build aplikasi:</p>

<pre><code>cd /var/www/lms/frontend
npm run build</code></pre>

<p>Jika build gagal, jangan restart PM2 dulu. Perbaiki error build terlebih dahulu.</p>

<h3>Langkah 9 — Restart LMS di PM2</h3>
<p>Jika build berhasil:</p>

<pre><code>pm2 restart lms
pm2 status
pm2 logs lms --lines 80</code></pre>

<p>Jika process <code>lms</code> belum ada:</p>
...
<h3>Langkah 10 — Validasi aplikasi internal</h3>
<p>Cek apakah aplikasi berjalan di port internal:</p>

<pre><code>curl -I http://localhost:3000</code></pre>

<p>Jika berhasil, berarti aplikasi berjalan di belakang Nginx.</p>

<h3>Langkah 11 — Validasi Nginx</h3>
<p>Cek konfigurasi Nginx:</p>
...
<p>Jika config berubah, reload:</p>

<pre><code>sudo systemctl reload nginx</code></pre>

<h3>Langkah 12 — Validasi domain production</h3>
<p>Cek domain HTTPS:</p>
...
<h3>Langkah 13 — Restore database jika nanti sudah ada</h3>
<p>Jika LMS belum memakai database sendiri, langkah ini bisa dilewati.</p>
<p>Jika nanti memakai PostgreSQL, restore dilakukan dengan hati-hati di environment yang benar.</p>

<pre><code>cd /var/backups/lms
gunzip -c lms-db-YYYYMMDD-HHMM.sql.gz | psql -h localhost -U nama_user_db -d nama_database</code></pre>

<p>Untuk MySQL/MariaDB:</p>

<pre><code>cd /var/backups/lms
gunzip -c lms-db-YYYYMMDD-HHMM.sql.gz | mysql -u nama_user_db -p nama_database</code></pre>

<p><strong>Penting:</strong> restore database bisa menimpa data. Untuk production, lakukan hanya jika yakin file backup benar dan target database tepat.</p>

<h3>Langkah 14 — Checklist setelah restore</h3>
<table>
  <thead>
    <tr>
      <th>Area</th>
  ...
  <tbody>
    <tr>
      <td>Source</td>
      <td><code>ls -la /var/www/lms</code></td>
      <td>File project ada</td>
    </tr>
    <tr>
      <td>Env</td>
      <td><code>ls -la | grep env</code></td>
      <td>Env production tersedia</td>
    </tr>
    <tr>
      <td>Build</td>
...
<h4>1. Setelah restore muncul 502 Bad Gateway</h4>
<p>Biasanya aplikasi belum berjalan di port internal.</p>

<pre><code>pm2 status
curl -I http://localhost:3000
pm2 logs lms --lines 100</code></pre>

<h4>2. npm install gagal</h4>
<p>Pastikan berada di folder frontend dan ada <code>package.json</code>:</p>

<pre><code>cd /var/www/lms/frontend
ls -la
npm -v</code></pre>

<h4>3. npm run build gagal</h4>
<p>Cek error paling atas dan paling bawah di terminal.</p>
<p>Sering terjadi karena dependency belum lengkap, env belum benar, atau file source belum lengkap.</p>

<pre><code>cd /var/www/lms/frontend
npm run build</code></pre>

<h4>4. Env hilang setelah clone ulang</h4>
<p>File env biasanya tidak ikut Git. Buat ulang dari contoh:</p>

<pre><code>cd /var/www/lms/frontend
cp .env.example .env.production
nano .env.production</code></pre>

<h4>5. Git clone gagal permission denied</h4>
<p>Cek SSH GitHub/GitLab:</p>

<pre><code>ssh -T git@github.com</code></pre>

<p>Jika belum berhasil, setup SSH key terlebih dahulu.</p>

<h4>6. Restore folder salah lokasi</h4>
<p>Cek posisi folder:</p>
...
<p>Pastikan struktur akhirnya tetap seperti:</p>

<pre><code>/var/www/lms/frontend/package.json</code></pre>

<h4>7. Restore database salah target</h4>
<p>Stop dulu dan cek ulang nama database, user, host, dan file backup. Jangan restore database production jika belum yakin.</p>

<pre><code>ls -lh /var/backups/lms
psql -h localhost -U nama_user_db -d nama_database</code></pre>
...
curl -I https://domainkita.com
curl -I http://localhost:3000
ls -la /var/www | grep lms-backup
cd /var/www
mv lms lms-broken-$(date +%Y%m%d-%H%M)
cp -r lms-backup-YYYYMMDD-HHMM lms
cd /var/www/lms/frontend
ls -la | grep env
npm install
npm run build
...
<h3>Kesimpulan</h3>
<p>Pada lesson ini, kita belajar restore dasar LMS setelah terjadi masalah.</p>
<p>Urutan paling aman adalah cek kondisi, amankan folder rusak, restore source, pastikan env, install dependency, build, restart PM2, cek Nginx, lalu validasi domain.</p>
<p>Rule pentingnya:</p>

<pre><code>backup tanpa restore plan = belum aman
restore tanpa validasi = belum selesai</code></pre>`,
          },
          {
            id: "l2l-41-video",
            title: "Video Restore Dasar",
            type: "video",
            description:
              "Video pendamping untuk memahami skenario restore dasar LMS setelah incident.",
            url: "https://youtu.be/uOeAt_woF_c?si=v5zNPGajvaOKYyrJ",
            duration: "32 min",     
          },
        ],
      },
      {
        ...
        duration: "32 min",
        summary:
          "Menjadikan course ini sebagai living course yang terus diperbarui berdasarkan error nyata, catatan deployment, troubleshooting, dan pembelajaran production LMS.",  
        order: 5,
        materials: [
          {
            id: "l2l-42-html",
            ...
            type: "html",
            description:
              "Membuat catatan living course, mencatat error nyata deployment, membuat changelog, dan mengubah pengalaman troubleshooting menjadi materi belajar.", 
            htmlContent:
              `<h2>Living Course Notes</h2>
<p>Pada lesson terakhir ini, kita akan menjadikan course <strong>From Localhost to Live LMS</strong> sebagai living course.</p>
<p>Artinya, materi ini tidak berhenti setelah LMS berhasil live. Setiap error nyata, troubleshooting, solusi, dan improvement production bisa dicatat lalu dijadikan pembaruan materi.</p>

<h3>Apa itu living course?</h3>
<p><strong>Living course</strong> adalah course yang terus bertumbuh berdasarkan pengalaman nyata.</p>
<p>Dalam konteks deployment LMS, living course berarti kita tidak hanya mengajar teori, tetapi juga mencatat kejadian real seperti DNS error, Nginx salah config, PM2 mati, SSL gagal, cache tidak muncul, atau restore gagal.</p>

<h3>Kenapa living notes penting?</h3>
<ul>
  <li>Materi tetap relevan dengan kasus nyata.</li>
  <li>Error yang pernah terjadi tidak hilang begitu saja.</li>
  <li>Peserta berikutnya bisa belajar dari masalah real deployment.</li>
  <li>Course menjadi semakin kuat dari waktu ke waktu.</li>
  <li>Instruktur punya dokumentasi troubleshooting yang rapi.</li>
</ul>

<h3>Folder kerja command</h3>
<p>Living notes bisa dibuat sebagai file dokumentasi di dalam project LMS.</p>
<p>Gunakan folder project:</p>
...
<h3>Langkah 1 — Buat folder notes</h3>
<p>Command ini dijalankan di VPS atau di local development, tergantung tempat kita mengelola source course.</p>

<pre><code>cd /var/www/lms
mkdir -p docs/deployment-notes</code></pre>

<p>Cek hasilnya:</p>

<pre><code>ls -la docs
ls -la docs/deployment-notes</code></pre>

<h3>Langkah 2 — Buat file living notes</h3>
<p>Buat file catatan utama:</p>

<pre><code>cd /var/www/lms
nano docs/deployment-notes/living-course-notes.md</code></pre>

<p>Jika <code>nano</code> belum tersedia:</p>

<pre><code>sudo apt update
sudo apt install nano -y</code></pre>

<p>Contoh isi awal:</p>

<pre><code># Living Course Notes
...
- Apakah materi course perlu diperbarui:

## Catatan Error Nyata

### 2026-04-27 — Contoh
- Lesson terkait: Nginx Reverse Proxy
- Masalah: Domain menampilkan default Nginx page.
- Penyebab: Default site masih aktif.
- Solusi: Hapus symlink default dari sites-enabled, test nginx, reload nginx.
- Command:
  - sudo rm /etc/nginx/sites-enabled/default
  - sudo nginx -t
  - sudo systemctl reload nginx
- Update course: Tambahkan troubleshooting default site.</code></pre>

<h3>Langkah 3 — Simpan catatan error dengan format konsisten</h3>
<p>Setiap kali terjadi error, gunakan pola yang sama agar mudah dicari.</p>

<pre><code>## YYYY-MM-DD — Judul Masalah
...
- Solusi:
- Command validasi:
- Update materi yang diperlukan:</code></pre>

<h3>Langkah 4 — Buat changelog course</h3>
<p>Selain notes error, kita juga bisa membuat changelog.</p>

<pre><code>cd /var/www/lms
nano docs/deployment-notes/course-changelog.md</code></pre>

<p>Contoh isi:</p>

<pre><code># Course Changelog

## 2026-04-27

- Update lesson l2l-24 DNS ke VPS:
  - Menambahkan catatan nslookup dengan DNS publik 8.8.8.8.
  - Menambahkan troubleshooting Query refused dari router lokal.

- Update lesson l2l-28 Aktifkan HTTPS:
  - Menambahkan penjelasan bahwa email Certbot bukan tanda layanan berbayar.</code></pre>

<h3>Langkah 5 — Cek perubahan file</h3>
<p>Jika project memakai Git, cek perubahan:</p>
...
<h3>Langkah 6 — Commit living notes</h3>
<p>Setelah catatan rapi, commit ke repository:</p>

<pre><code>cd /var/www/lms
git add docs/deployment-notes
git commit -m "add living course deployment notes"
git push</code></pre>

<p>Dengan cara ini, catatan course ikut tersimpan di repository dan bisa dilacak perubahannya.</p>

<h3>Langkah 7 — Hubungkan notes dengan update lesson</h3>
<p>Setiap catatan error sebaiknya berakhir dengan keputusan:</p>
<ul>
  <li>Apakah lesson perlu diperbarui?</li>
  <li>Apakah perlu section troubleshooting baru?</li>
  <li>Apakah perlu command validasi tambahan?</li>
  <li>Apakah perlu warning agar peserta tidak salah langkah?</li>
</ul>

<h3>Langkah 8 — Contoh mapping error ke lesson</h3>
<table>
  <thead>
    <tr>
      <th>Error nyata</th>
      <th>Lesson yang perlu diupdate</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>DNS <code>Query refused</code></td>
      <td>DNS ke VPS</td>
    </tr>
    <tr>
      <td>Default Nginx page muncul</td>
      <td>Enable Site Config</td>
    </tr>
    <tr>
      <td>502 Bad Gateway</td>
      <td>Run LMS via PM2 / Monitoring PM2</td>
    </tr>
    <tr>
      <td>Certbot gagal challenge</td>
      <td>Aktifkan HTTPS</td>
    </tr>
    <tr>
      <td>Env berubah tapi website belum berubah</td>
      <td>Environment Production</td>
    </tr>
  </tbody>
</table>

<h3>Langkah 9 — Review berkala</h3>
<p>Living notes sebaiknya direview secara berkala, misalnya setelah:</p>
<ul>
  <li>Deployment besar.</li>
  <li>Update Nginx atau PM2.</li>
  <li>Perubahan domain atau SSL.</li>
  <li>Incident production.</li>
  <li>Feedback dari peserta course.</li>
</ul>

<h3>Catatan untuk pemula</h3>
<p>Jangan takut mencatat error. Error adalah bahan belajar yang sangat berharga.</p>
<p>Yang penting bukan hanya error-nya hilang, tetapi kita tahu penyebabnya dan bisa menjelaskan solusinya ke orang lain.</p>

<h3>Catatan untuk advanced</h3>
<p>Untuk course yang lebih matang, living notes bisa dikembangkan menjadi:</p>
<ul>
  <li>Incident log.</li>
  <li>Deployment runbook.</li>
  <li>Knowledge base internal.</li>
  <li>Troubleshooting matrix.</li>
  <li>Postmortem ringan setiap incident.</li>
</ul>

<h3>Troubleshooting</h3>

<h4>1. Folder docs belum ada</h4>
<p>Buat foldernya:</p>

<pre><code>cd /var/www/lms
mkdir -p docs/deployment-notes</code></pre>

<h4>2. Permission denied saat membuat notes</h4>
<p>Cek owner folder:</p>

<pre><code>ls -la /var/www/lms</code></pre>

<p>Jika perlu, ubah owner ke user deploy:</p>

<pre><code>sudo chown -R $USER:$USER /var/www/lms</code></pre>

<h4>3. nano tidak tersedia</h4>
<p>Install nano:</p>

<pre><code>sudo apt update
sudo apt install nano -y</code></pre>

<h4>4. Git commit gagal karena belum ada user name/email</h4>
<p>Set Git identity:</p>

<pre><code>git config --global user.name "Nama Kita"
git config --global user.email "email@example.com"</code></pre>

<h4>5. Git push gagal</h4>
<p>Cek remote dan koneksi SSH:</p>

<pre><code>git remote -v
ssh -T git@github.com</code></pre>

<h4>6. Catatan terlalu panjang dan sulit dicari</h4>
<p>Pisahkan notes berdasarkan kategori:</p>

<pre><code>docs/deployment-notes/dns.md
docs/deployment-notes/nginx.md
docs/deployment-notes/pm2.md
docs/deployment-notes/ssl.md
docs/deployment-notes/restore.md</code></pre>

<h3>Ringkasan command</h3>

<pre><code>cd /var/www/lms
mkdir -p docs/deployment-notes
nano docs/deployment-notes/living-course-notes.md
nano docs/deployment-notes/course-changelog.md
ls -la docs/deployment-notes
git status
git add docs/deployment-notes
git commit -m "add living course deployment notes"
git push
...
<h3>Kesimpulan</h3>
<p>Pada lesson ini, kita menjadikan course sebagai living course yang terus bertumbuh berdasarkan pengalaman real deployment.</p>
<p>Setiap error nyata sebaiknya diubah menjadi catatan, troubleshooting, dan update materi.</p>
<p>Rule akhirnya sederhana:</p>

<pre><code>error nyata → catat
solusi berhasil → dokumentasikan
lesson terkait → update</code></pre>`,
          },
          {
            id: "l2l-42-video",
            title: "Video Living Course Notes",
            type: "video",
            description:
              "Video pendamping untuk memahami cara menjadikan course deployment LMS sebagai living course.",
            url: "https://youtu.be/uOeAt_woF_c?si=v5zNPGajvaOKYyrJ",
            duration: "30 min",
          },
        ],
...
export const fromLocalhostToLiveLmsCourse: Course = {
  id: "5",
